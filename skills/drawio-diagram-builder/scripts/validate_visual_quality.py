...
Catches computable visual defects — 10px grid violations, arrow-box collisions,
text overflow risk, font proportionality, spacing variance, color incoherence,
decoration blocks, orphans, font-size anomalies, edge density — BEFORE the first screenshot.
...
def _is_grid_aligned(value: float, grid_size: int = GRID_SIZE) -> bool:
    return abs(value / grid_size - round(value / grid_size)) < 1e-6
...
def check_grid_alignment(vertices: List[Vertex], edges: List[Edge]) -> List[Finding]:
    """Enforce the mandatory 10px grid for authored geometry."""
    findings = []
    ...
    for v in vertices:
        ...
        for name, value in (
            ...
            if not _is_grid_aligned(value):
                bad_attrs.append(f"{name}={value:g}")
        if bad_attrs:
            findings.append(Finding(
                ...
                element_id=v.id,
                message=(
                    f"Element '{v.id}' is not aligned to the mandatory 10px grid "
                    f"({', '.join(bad_attrs)}). Round position and size values to multiples of 10."
                ),
                detail={"bad_attrs": bad_attrs, "grid_size": GRID_SIZE},
            ))

    for e in edges:
        points = []
        if e.source_point is not None:
            points.append(("sourcePoint", e.source_point))
        for idx, point in enumerate(e.waypoints, start=1):
            points.append((f"waypoint{idx}", point))
        if e.target_point is not None:
            points.append(("targetPoint", e.target_point))

        bad_points = []
        for label, (x, y) in points:
            bad_attrs = []
            if not _is_grid_aligned(x):
                bad_attrs.append(f"x={x:g}")
            if not _is_grid_aligned(y):
                bad_attrs.append(f"y={y:g}")
            if bad_attrs:
                bad_points.append(f"{label}({', '.join(bad_attrs)})")
        ...
        if bad_points:
            findings.append(Finding(
                ...
                element_id=e.id,
                message=(
                    f"Edge '{e.id}' has explicit route points off the mandatory 10px grid "
                    f"({', '.join(bad_points)}). Round all mxPoint coordinates to multiples of 10."
                ),
                detail={"bad_points": bad_points, "grid_size": GRID_SIZE},
            ))
...
ALL_RULES = [
    ("grid-alignment", check_grid_alignment),
...
def run_all_checks(
    ...
    for rule_name, rule_fn in ALL_RULES:
        ...
        elif rule_name in ("arrow-box-collision", "orphan-labels", "grid-alignment"):
