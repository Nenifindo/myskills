...
All authored geometry must stay on a 10px grid. For every drawn control, element, module, label box, icon container, repeated primitive, and connector route:

- Use only integer multiples of 10 for vertex `mxGeometry` `x`, `y`, `width`, and `height`.
- Use only integer multiples of 10 for explicit edge `mxPoint` `x` and `y` coordinates, including `sourcePoint`, `targetPoint`, and waypoint points under `Array as="points"`.
- Round measured or reference-derived coordinates/sizes to the nearest 10 before writing XML.
- Keep small icons and decorative primitives grid-aligned too; for example use `20x20`, `30x30`, or `40x40`, not `24x24`.
- Draw.io relative edge geometry such as `<mxGeometry relative="1" as="geometry" />` is allowed when it has no absolute coordinates.
...
```xml
<mxGeometry x="123" y="78" width="245" height="63" as="geometry" />
<mxPoint x="357.5" y="112" as="targetPoint" />
```
