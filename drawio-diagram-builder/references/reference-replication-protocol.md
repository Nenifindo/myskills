...
13. Quantize all final geometry to the mandatory 10px grid: every `x`, `y`, `w`, `h`, icon box, label box, repeated component, and explicit connector point must be a multiple of 10.
...
- scale assumption:
- margin:
- final grid: 10px mandatory; all x/y/w/h and explicit connector points are multiples of 10
