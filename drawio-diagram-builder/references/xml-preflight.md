...
### 0. 10px Grid Alignment (FAIL)

Every authored geometry value must align to the mandatory 10px grid:
- Vertex `mxGeometry` `x`, `y`, `width`, and `height` must be integer multiples of 10.
- Explicit edge `mxPoint` `x` and `y` coordinates, including source points, target points, and waypoints, must be integer multiples of 10.
- Relative edge geometry with no absolute coordinates is allowed.

This catches: arbitrary coordinates like `137`, fractional control points like `222.5`, icon boxes like `24x24`, and module sizes that break grid rhythm.

Fix: round positions, sizes, and explicit connector points to the nearest 10, then re-run pre-flight.
