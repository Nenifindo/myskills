...
**10-pixel geometry grid is mandatory.** Every drawn control, element, module, label box, icon container, and connector route point must use geometry aligned to 10px increments: `x`, `y`, `width`, and `height` values must be integer multiples of 10; explicit edge `mxPoint` coordinates must also be integer multiples of 10. Plan layout on the 10px grid and round reference-derived coordinates/sizes to the nearest 10 before writing XML. The only exception is draw.io's internal relative edge geometry that has no absolute coordinate attributes.
...
3. **Build the diagram brief and visual specification**
   ...
   - Quantize the coordinate plan to the mandatory 10px grid: all positions, sizes, icon boxes, label boxes, repeated modules, and explicit connector waypoints must be multiples of 10.
...
4. **Author the `.drawio` file**
   ...
   - Keep every absolute `mxGeometry` `x`, `y`, `width`, and `height` value, plus every explicit edge `mxPoint` coordinate, on the 10px grid. Do not use arbitrary values such as `137`, `222.5`, or `43`; use `140`, `220`, or `40`.
