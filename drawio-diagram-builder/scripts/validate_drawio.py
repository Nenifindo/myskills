...
def is_grid_aligned(value: float, grid_size: int = GRID_SIZE) -> bool:
...
def main() -> int:
    ...
    for cell in cells:
        ...
        if geometry is not None:
            for attr in ("x", "y", "width", "height"):
                value = numeric(geometry.attrib.get(attr))
                if value is not None and not is_grid_aligned(value):
                    grid_violations.append(f"{cell_id} mxGeometry {attr}={value:g}")
            for point in geometry.iter("mxPoint"):
                point_as = point.attrib.get("as", "point")
                for attr in ("x", "y"):
                    value = numeric(point.attrib.get(attr))
                    if value is not None and not is_grid_aligned(value):
                        grid_violations.append(f"{cell_id} mxPoint[{point_as}] {attr}={value:g}")
    ...
    if grid_violations:
        errors.append(
            "geometry is not aligned to the mandatory 10px grid: "
            + ", ".join(grid_violations[:20])
        )
    ...
    stats = {
        ...
        "grid_violation_count": len(grid_violations),
    ...
    print(f"grid_violation_count: {stats['grid_violation_count']}")
